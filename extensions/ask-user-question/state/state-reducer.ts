import type { QuestionAnswer, QuestionData, QuestionnaireResult } from "../tool/types";
import type { WrappingSelectItem } from "../view/components/wrapping-select";
import type { QuestionnaireAction } from "./key-router";
import { ROW_INTENT_META } from "./row-intent";
import { computeFocusedOptionHasPreview } from "./selectors/derivations";
...
function withPreviewAndNotes(
	answer: QuestionAnswer,
	ctx: ApplyContext,
	notesByTab: ReadonlyMap<number, string>,
): QuestionAnswer {
	let next = answer;
	if (next.kind === "option" && next.answer) {
		const q = ctx.questions[next.questionIndex];
		const matched = q?.options.find((o) => o.label === next.answer);
		if (matched?.preview && matched.preview.length > 0) {
			next = { ...next, preview: matched.preview };
		}
	}
	const pendingNotes = notesByTab.get(next.questionIndex);
	if (pendingNotes && pendingNotes.length > 0) {
		next = { ...next, notes: pendingNotes };
	}
	return next;
}

function commitAnswer(state: QuestionnaireState, answer: QuestionAnswer, ctx: ApplyContext): QuestionnaireState {
	const answers = new Map(state.answers);
	answers.set(answer.questionIndex, withPreviewAndNotes(answer, ctx, state.notesByTab));
	return { ...state, answers };
}

const confirmHandler: Handler<"confirm"> = (state, action, ctx) => {
	const next = commitAnswer(state, action.answer, ctx);
	if (action.answer.kind === "chat") return doneFor(next, ctx, false);
...
function persistNotesDraft(state: QuestionnaireState): QuestionnaireState {
	...
	return { ...state, notesByTab: notes, answers, notesVisible: false };
}

const notesExitHandler: Handler<"notes_exit"> = (state, _action, _ctx) => ({
	state: persistNotesDraft(state),
	effects: [{ kind: "set_notes_focused", focused: false }],
});

const notesConfirmHandler: Handler<"notes_confirm"> = (state, action, ctx) => {
	const withNotes = persistNotesDraft(state);
	const next = commitAnswer(withNotes, action.answer, ctx);
	const effects: Effect[] = [{ kind: "set_notes_focused", focused: false }];
	const result = action.answer.kind === "chat"
		? doneFor(next, ctx, false)
		: action.autoAdvanceTab !== undefined
			? switchTabResult(next, action.autoAdvanceTab, ctx)
			: doneFor(next, ctx, false);
...
const HANDLERS: { [K in QuestionnaireAction["kind"]]: Handler<K> } = {
	...
	notes_confirm: notesConfirmHandler,
