...
import type { QuestionAnswer } from "../tool/types";
import { ROW_INTENT_META } from "./row-intent";
import type { QuestionnaireRuntime, QuestionnaireState } from "./state";
...
export type QuestionnaireAction =
	...
	| { kind: "notes_enter" }
	| { kind: "notes_exit" }
	| { kind: "notes_confirm"; answer: QuestionAnswer; autoAdvanceTab?: number }
	| { kind: "submit" }
	...
	if (state.notesVisible) {
		...
		if (kb.matches(data, KEYBIND_CONFIRM)) {
			...
			const answer = q?.multiSelect
				...
				: buildSingleSelectAnswer(state, runtime);
			if (!answer) return { kind: "notes_exit" };
			return { kind: "notes_confirm", answer, autoAdvanceTab: computeAutoAdvanceTab(state, runtime) };
		}
